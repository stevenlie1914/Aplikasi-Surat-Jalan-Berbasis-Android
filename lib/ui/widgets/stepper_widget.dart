...
class _StepperState extends State<Stepper> with TickerProviderStateMixin {
  ...
  Widget _buildLastPoint(int index, bool oldState) {
    return Container(
      margin: const EdgeInsets.symmetric(vertical: 5.0),
      width: _kStepSize,
      height: _kStepSize,
  ...
  Widget _buildIcon(int index) {
    return index == widget.steps.length - 1 ? _buildLastPoint(index, false) : _buildCircle(index, false);
  }
  ...
  Widget _buildVertical() {
    return ListView(
      ...
      children: <Widget>[for (int i = 0; i < widget.steps.length; i += 1) _buildVerticalHeader(i)],
    );
  }
